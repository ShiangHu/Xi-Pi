function [output,p] = my_csaps(x,y,p,xx,w)
...
if n==2 % the smoothing spline is the straight line interpolant
   ...
   if p<0 % we are to determine an appropriate P
      ...
      u= double(((6*(1-p))*QtWQ+p*R))\double(diff(divdif));
   else
      u= double(((6*(1-p))*(Qtw*Qtw.')+p*R))\double(diff(divdif));
   end
   % ... and convert to pp form
   % Qt.'*u=diff([0;diff([0;u;0])./dx;0])
   yi = yi - ...
    (6*(1-p))*W*double(diff([zeros(1,yd)
